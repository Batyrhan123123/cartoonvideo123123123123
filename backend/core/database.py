...
class DatabaseManager:
    ...
    # Parameters that asyncpg.connect() actually accepts (from asyncpg source)
    _ASYNCPG_SUPPORTED_PARAMS = frozenset({
        "host", "port", "user", "password", "passfile", "service", "servicefile",
        "database", "timeout", "statement_cache_size", "max_cached_statement_lifetime",
        "max_cacheable_statement_size", "command_timeout", "ssl", "direct_tls",
        "connection_class", "record_class", "server_settings",
    })

    def _sanitize_asyncpg_url(self, database_url: str) -> str:
        """Remove unsupported query parameters from the database URL for asyncpg.

        SQLAlchemy's asyncpg dialect passes ALL URL query params to asyncpg.connect().
        Cloud providers (e.g., Supabase, Neon) may include params like 'channel_binding',
        'sslmode', etc. that asyncpg doesn't accept, causing TypeError at connect time.
        """
        import urllib.parse
        ...
        # Also allow 'sslmode' as SQLAlchemy handles it specially
        allowed_extras = {"sslmode"}
        filtered_params = {}
        removed_params = []

        for key, values in query_params.items():
            if key in self._ASYNCPG_SUPPORTED_PARAMS or key in allowed_extras:
        ...
        if removed_params:
            logger.warning(
                f"Removed unsupported asyncpg URL parameters: {removed_params}. "
                "These are not recognized by asyncpg.connect()."
            )
    ...
    async def init_db(self):
        ...
        try:
            ...
            # Fix for asyncpg compatibility:
            # SQLAlchemy's asyncpg dialect passes ALL URL query parameters directly
            # to asyncpg.connect() via opts.update(url.query). Some PostgreSQL
            # connection URLs (e.g., from cloud providers) include parameters like
            # 'channel_binding' that asyncpg doesn't support, causing TypeError.
            # Solution: Strip unsupported parameters from the URL before engine creation.
            if "+asyncpg" in database_url:
                database_url = self._sanitize_asyncpg_url(database_url)
                engine_kwargs["connect_args"] = {
                    "statement_cache_size": 0 if is_lambda else 100,
                }
