...
export default function NewPortfolioSection() {
  ...
  return (
    <section id="portfolio" className="relative py-24 sm:py-32 overflow-hidden">
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-[#9333EA]/15 rounded-full blur-[150px]" />
      ...
      <div ref={ref} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center mb-16 transition-all duration-700 ${isInView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">
            <span className="new-gradient-text">Работы</span>
          </h2>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-medium transition-all ${
                activeCategory === cat
                  ? "new-gradient-btn text-white shadow-lg shadow-[#9333EA]/25"
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((item, i) => (
            <div
              key={item.id}
              className={`group relative rounded-2xl overflow-hidden glass hover:border-[#9333EA]/30 transition-all duration-500 hover:scale-[1.02] hover:shadow-xl hover:shadow-[#9333EA]/10 ${
            ...
            >
              <div className="relative aspect-video overflow-hidden bg-black/50 cursor-pointer" onClick={() => togglePlay(item.id)}>
                ...
                <div className={`absolute inset-0 bg-black/30 transition-colors flex items-center justify-center ${playingId === item.id ? "opacity-0 hover:opacity-100" : "opacity-100"}`}>
                  <div className="w-14 h-14 rounded-full glass-strong flex items-center justify-center transition-transform hover:scale-110">
                    {playingId === item.id ? <Pause size={24} className="text-white" /> : <Play size={24} className="text-white ml-1" />}
                  </div>
                </div>
              ...
              <div className="p-5">
                <span className="text-xs font-medium text-[#A855F7] uppercase tracking-wider">{item.category}</span>
